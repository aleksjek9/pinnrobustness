...
def get_gradient(l2_lambda, indexes, results, y_train, control_variable, advection_variable,
    ...
    advection = float(advection_variable.tape_value())

    K = (
        float(l2_lambda * viscosity * advection) ** 2
        + assemble(
            sum(
    ...
    error = float(K)
    print(error)
    rf = ReducedFunctional(K, [control_variable, advection_variable])
    gradient = rf.derivative()

    gradient0 = float(gradient[0])
    gradient1 = float(gradient[1])
    
    print(gradient0, gradient1)

    return error, gradient0, gradient1


def burgers_1d(viscosity, advection, initial_condition, gradient_mode=False, excluded_indices = []):
    """Inspired by:
    ...
    advection = Constant(advection)
    control_variable = Control(viscosity)
    control_advection = Control(advection)
    ...
    F = (
        ...
        + advection * inner(cur_vel * cur_vel.dx(0), test_vel)
    ) * dx
    ...
    if gradient_mode:
        return [result], control_variable, control_advection, solution_comparison, dof_to_vertex_map(velocity), []
