...
class Model(nn.Module):
    ...
    def save_history(self, elapsed_minutes, phy_loss, cc_loss, val_loss, visc, adv):
        """
        ...
        self.advection_history.append(adv)
        ...
        if self.epoch in list(range(1, 10000, 100)):
            ...
            with open('results/advection_history_' + self.name + '.txt', 'a') as f:
                for item in self.advection_history:
                    f.write(f"{item}, ")
    ...
    def phy_loss(self, pde):
        ...
        pde_loss = y_t + ((10**self.adv)*torch.flatten(output) * y_x) - ((10**self.visc) * y_x2)
    ...
    def loss_fn(self, bc, ic, cc, val, pde):
        ...
        print("Epoch: ", self.epoch, " Minutes: ", elapsed_minutes, " Phy_loss: ", phy_loss.item(), " Data_loss: ", data_loss.item(), " Viscosity: ", 10**self.visc.item(), " Advection: ", 10**self.adv.item())
