...
def PINN_experiment(data, noise, verbose=True, rerun=False):
    ...
    for noise_level in noise:
        ...
        for sample in range(samples):
            ...
            fem_result = prepare_tensor(burgers_1d(viscosity, advection, initial_condition, excluded_indices = random_indices))
            ...
            noise_estimated_advection.append(advection)
            error = root_mean_squared_error([advection], [0.55])
            noise_advection_error.append(error)
            ...
            if verbose:
                ...
                print(noise_estimated_advection)
                print(noise_parameter_error)
                print(noise_advection_error)
    ...
    all_results = [np.array(rmse, dtype=object), np.array(estimated_parameter, dtype=object), np.array(parameter_error, dtype=object), np.array(fem_error, dtype=object), np.array(stats, dtype=object), np.array(estimated_advection, dtype=object), np.array(advection_error, dtype=object)]
