...
class Optimizer:
    ...
    def grad(self, parameters):
        ...
        print(parameters)
        return np.array([self.gradient, self.gradient1])


    def error(self, parameters):
        """Solves 1D Burgers. Then calculates and returns the error on training set."""

        viscosity, advection = parameters

        try:
            results, control_variable, control_advection, solution_comparison, map, domain = self.burgers_1d(
                viscosity, advection, self.initial_condition, gradient_mode=True
            )
        except:
            return np.nan

        error, self.gradient, self.gradient1 = self.get_gradient(
            self.l2_lambda, 
            self.indexes, 
            results, 
            self.y_train, 
    ...
    def error_include_val(self, parameters):
        """Solves 1D Burgers. Then calculates and returns the error on training set. 
    ...
    def validation(self):
        ...
        result = self.burgers_1d(self.viscosity, self.advection, self.initial_condition)
    ...
    def test(self):
        ...
        result = self.burgers_1d(self.viscosity, self.advection, self.initial_condition, excluded_indices=inds)
