...
class Model(nn.Module):
    ...
    def create_network(self):
        ...
        network.append(nn.Linear(4, 20).double())
        ...
        network.append(nn.Linear(20, 20).double())
        network.append(nn.Tanh().double())
        network.append(nn.Linear(20, 4).double())
    ...
    def save_history(self, elapsed_minutes, phy_loss, cc_loss, val, visc):
        ...
        if self.epoch in list(range(1, 300002, 10000)):
            #Only saves progress ever 10 000 epochs

            with open('results/minutes.txt', 'a') as f:
                for item in self.minutes:
                    f.write(f"{item}, ")

            with open('results/phy_history.txt', 'a') as f:
                for item in self.phy_history:
                    f.write(f"{item}, ")

            with open('results/data_history.txt', 'a') as f:
                for item in self.data_history:
                    f.write(f"{item}, ")

            val_loss = [self.mse_loss(val)]

            with open('results/val_history.txt', 'a') as f:
                for item in val_loss:
                    f.write(f"{item}, ")

            with open('results/parameter_history.txt', 'a') as f:
    ...
    def phy_loss(self, pde):
        ...
        x.requires_grad, y.requires_grad, z.requires_grad, t.requires_grad = True, True, True, True
        pde = torch.stack((x, y, z, t), dim=1)
        ...
        u_z = gradient(u_pred, z)
        u_xx = gradient(u_x, x, create=False)
        u_yy = gradient(u_y, y, create=False)
        u_zz = gradient(u_z, z, create=False)

        v_t = gradient(v_pred, t, create=False)
        v_x = gradient(v_pred, x)
        v_y = gradient(v_pred, y)
        v_z = gradient(v_pred, z)
        v_xx = gradient(v_x, x, create=False)
        v_yy = gradient(v_y, y, create=False)
        v_zz = gradient(v_z, z, create=False)

        w_t = gradient(w_pred, t, create=False)
        w_x = gradient(w_pred, x)
        w_y = gradient(w_pred, y)
        w_z = gradient(w_pred, z)
        w_xx = gradient(w_x, x, create=False)
        w_yy = gradient(w_y, y, create=False)
        w_zz = gradient(w_z, z, create=False)

        p_x = gradient(p_pred, x, create=False)
        p_y = gradient(p_pred, y, create=False)
        p_z = gradient(p_pred, z, create=False)

        pde_loss1 = u_x + v_y + w_z
        pde_loss2 = u_t + u_pred*u_x + v_pred*u_y + w_pred*u_z + p_x - viscosity *(u_xx+u_yy+u_zz)
        pde_loss3 = v_t + u_pred*v_x+v_pred*v_y+w_pred*v_z+p_y-viscosity*(v_xx + v_yy + v_zz)
        pde_loss4 = w_t + u_pred*w_x+v_pred*w_y+w_pred*w_z+p_z-viscosity*(w_xx + w_yy + w_zz)

        loss = pde_loss1 +pde_loss2 + pde_loss3 + pde_loss4
